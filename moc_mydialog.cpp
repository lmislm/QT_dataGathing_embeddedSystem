/****************************************************************************
** Meta object code from reading C++ file 'mydialog.h'
**
** Created: Thu Mar 2 13:18:48 2017
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "mydialog.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'mydialog.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_mydialog[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      10,    9,    9,    9, 0x05,

 // slots: signature, parameters, type, tag, flags
      23,    9,    9,    9, 0x08,
      42,    9,    9,    9, 0x08,
      61,    9,    9,    9, 0x08,
      87,    9,    9,    9, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_mydialog[] = {
    "mydialog\0\0sig(QString)\0on_radio_clicked()\0"
    "on_check_clicked()\0on_combo_changed(QString)\0"
    "on_Yes_clicked()\0"
};

const QMetaObject mydialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_mydialog,
      qt_meta_data_mydialog, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &mydialog::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *mydialog::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *mydialog::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_mydialog))
        return static_cast<void*>(const_cast< mydialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int mydialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: sig((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 1: on_radio_clicked(); break;
        case 2: on_check_clicked(); break;
        case 3: on_combo_changed((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 4: on_Yes_clicked(); break;
        default: ;
        }
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void mydialog::sig(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
